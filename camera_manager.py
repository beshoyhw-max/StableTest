...
class MPCameraThread(threading.Thread):
    ...
    def __init__(self, camera_config, conf_threshold=0.25, 
        ...
        # Frame ring buffer: stores frames keyed by frame_count
        # This ensures we match detection results with the EXACT frame they were detected on
        # (Fixes frame-box temporal mismatch bug in absence evidence)
        self._frame_buffer = {}  # {frame_count: frame_copy}
        self._frame_buffer_max_size = 30  # Keep last 30 frames (~1 sec at 30fps)
    ...
    def run(self):
        ...
        while self.running:
            ...
            # Store frame in ring buffer keyed by frame_count
            # This ensures we can match detection results to their EXACT source frame
            with self._display_data_lock:  # Reuse existing lock for simplicity
                self._frame_buffer[frame_count] = raw_frame.copy()
                # Cleanup old frames to prevent OOM
                if len(self._frame_buffer) > self._frame_buffer_max_size:
            ...
            if result:
                ...
                if recognized_persons and self.attendance_tracker and result_ts > self._last_result_timestamp:
                    ...
                    # FIX: Get the EXACT frame that these boxes were detected on
                    # This prevents frame-box temporal mismatch in absence evidence
                    result_frame_count = result.get('frame_count', 0)
                    with self._display_data_lock:
                        cached_frame = self._frame_buffer.get(result_frame_count, raw_frame)
