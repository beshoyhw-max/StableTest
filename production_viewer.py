...
def launch_production_mode(camera_manager=None):
    ...
    try:
        ...
        if camera_manager:
            ...
            # Stop monitor threads FIRST to prevent false absence alerts
            camera_manager.stop_fps_monitor()
            
            # Clear attendance tracker to avoid stale data triggering alerts
            if hasattr(camera_manager, 'attendance_tracker') and camera_manager.attendance_tracker:
                camera_manager.attendance_tracker.tracked_people.clear()
                camera_manager.attendance_tracker.absence_events.clear()
                print("[Production] Cleared attendance tracker state")
...
def stop_production_mode():
    ...
    # Graceful shutdown via signal file
    STOP_SIGNAL_FILE = ".stop_production"
    
    script_dir = os.path.dirname(os.path.abspath(__file__))
    stop_file_path = os.path.join(script_dir, STOP_SIGNAL_FILE)
    
    try:
        print("[Production] Sending graceful stop signal...")
        with open(stop_file_path, 'w') as f:
            f.write("STOP")
            
        # Wait up to 5 seconds for graceful exit
        for _ in range(10):
            if _production_process.poll() is not None:
                print("[Production] Process exited gracefully.")
                _production_process = None
                break
            time.sleep(0.5)
        ...
        print(f"[Production] Error sending stop signal: {e}")

    # Force kill if still running
    if _production_process is not None:
        print("[Production] Process still running, forcing termination...")
        try:
            ...
            try:
                print("[Production] Terminate failed, killing...")
    ...
    if _saved_camera_manager:
        try:
            ...
            # Restart all cameras and monitors
            _saved_camera_manager.start_all_cameras()
